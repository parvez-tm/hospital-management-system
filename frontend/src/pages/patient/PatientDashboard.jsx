...
import { FiCalendar, FiUsers, FiHeart, FiThermometer, FiWind } from "react-icons/fi";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between py-2 border-b border-gray-50">
              ...
              <span className="font-medium text-gray-800">{user?.contactNo}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-50">
              <span className="text-gray-500">Age</span>
              <span className="font-medium text-gray-800">{user?.age || "N/A"}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-50">
              <span className="text-gray-500">Height</span>
              <span className="font-medium text-gray-800">{user?.height || "N/A"}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-gray-500">Weight</span>
              <span className="font-medium text-gray-800">{user?.weight || "N/A"}</span>
            </div>
          </div>
        </div>
        ...
        {/* Latest Vitals */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Latest Vitals</h3>
          {lv ? (
            <>
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="text-center p-4 bg-red-50 rounded-xl">
                  ...
                  <p className="text-lg font-bold text-gray-800">{lv.systolic}/{lv.diastolic}</p>
                ...
                <div className="text-center p-4 bg-blue-50 rounded-xl">
                  ...
                  <p className="text-lg font-bold text-gray-800">{lv.oxygen}%</p>
                  <p className="text-xs text-gray-400">SpO₂</p>
                </div>
                <div className="text-center p-4 bg-orange-50 rounded-xl">
                  <FiThermometer className="mx-auto text-orange-500 text-xl mb-1" />
                  <p className="text-xs text-gray-500">Temperature</p>
                  <p className="text-lg font-bold text-gray-800">{lv.temperature}°F</p>
                  <p className="text-xs text-gray-400">Body temp</p>
                </div>
              </div>
              <p className="text-xs text-gray-400">
                Recorded on {new Date(lv.createdAt).toLocaleDateString("en-US", {
      ...
      <div className="mt-6 bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl shadow-lg p-6 text-white">
        ...
        <div className="flex flex-wrap gap-3 mt-3">
          <a href="/patient/vitals" className="bg-white/10 hover:bg-white/20 rounded-lg px-5 py-3 text-sm font-medium transition-colors">
            → View All Vitals
          </a>
          <a href="/patient/profile" className="bg-white/10 hover:bg-white/20 rounded-lg px-5 py-3 text-sm font-medium transition-colors">
            → Edit Profile
          </a>
        </div>
      </div>
